...
void Renderer::DrawObjects()
{
    for (Object object : objects)
    {
        if (camera.IsVisible(object.GetPosition()))
        {
            ...
            glUniform3f(glGetUniformLocation(shaderProgram, "lightColor"), lightSource.GetColor().r, lightSource.GetColor().g, lightSource.GetColor().b);
            ...
            glUniform1f(glGetUniformLocation(shaderProgram, "lightStrength"), lightSource.GetParams(0));
            glUniform1f(glGetUniformLocation(shaderProgram, "lightAmbient"), lightSource.GetParams(1));
            glUniform1f(glGetUniformLocation(shaderProgram, "specularStrength"), lightSource.GetParams(2));
            glUniform1f(glGetUniformLocation(shaderProgram, "lightIntensityFalloff"), lightSource.GetParams(3));
            glUniform1f(glGetUniformLocation(shaderProgram, "lightEffectiveRangeInverse"), lightSource.GetParams(4));
            glUniform1f(glGetUniformLocation(shaderProgram, "lightInnerCone"), lightSource.GetParams(5));
            glUniform1f(glGetUniformLocation(shaderProgram, "lightOuterCone"), lightSource.GetParams(6));
